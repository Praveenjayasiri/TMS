...
<script>
  $(document).ready(function () {
    ...
    function loadData() {
      $.ajax({
        url: 'add_task/fetch_task_details.php',
        method: 'GET',
        dataType: 'json',
        success: function (data) {
          ...
          $.each(data, function (index, task) {
            var viewButtonHtml = task.task_status === 0
              ? '<button class="btn btn-primary btn-sm btn-view" data-id="' + task.id + '"><i class="fas fa-eye"></i> View</button>'
              : ''; // Empty string if emp_status is not 0
    ...
    $('#example1').on('click', '.btn-view', function () {
      ...
      window.location.href = 'selected_task_details.php?rowId=' + rowId;
    });
